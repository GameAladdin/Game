...
void Game::Update()
{
	...
	{		
		...
		this->_deviceManager->RenderMap(NULL, NULL);

		switch (this->mState)
		{
		case Engine::WALK:
		{
			this->mAladdin->Activities(Engine::WALK, Engine::RIGHT);
			if (this->mAladdin->GetCurrentLocation().x >= 700)
				this->mState = Engine::CLIMB;
			break;
		}
		case Engine::CLIMB:
		{
			this->mAladdin->Activities(Engine::CLIMB, Engine::UP);
			if (this->mAladdin->GetCurrentLocation().y <= 400)
				this->mState = Engine::SWING;
			break;
		}

		case Engine::SWING:
		{
			this->mAladdin->Activities(Engine::SWING, Engine::LEFT);
			if (this->mAladdin->GetCurrentLocation().x <= 300)
			{
				this->mState = Engine::DROP;
		...
		case Engine::DROP:
		{
			...
			this->mAladdin->Activities(Engine::DROP, Engine::RIGHT);
			if (this->mAladdin->GetCurrentLocation().y >= this->mAladdin->GetEndLocation().y)
			{
				this->mState = Engine::WALK;
			}
			break;
		}
...
void Game::InIt()
{
	...
	_gameTime->InIt();
	_deviceManager->Init(_graphics,"Map\\Map.png");


	LPD3DXSPRITE spriteHandler;

	if (D3DXCreateSprite(_deviceManager->getDevice(), &spriteHandler) != D3D_OK)
	{
		MessageBox(NULL, "Can't create spritehandler!", "Error", MB_OK | MB_ERR_INVALID_CHARS);
		return;
	}

	Engine::SetSpriteHandler(spriteHandler);
	...
	//Deadline
	this->mAladdin = new ALADDIN(D3DXVECTOR3(50, 500, 0));
